...
public class ArrayAllocation implements Expression {
	...
	public Type getType() {
		if (this.size.getType().compatibleWith(AtomicType.IntegerType)) {
			ArrayType arrayType = new ArrayType(this.element);
			Logger.info("ArrayAllocation: ArrayType created with element type " + this.element);
			return arrayType;
		} else {
			Logger.error("ArrayAllocation: Size expression is not compatible with IntegerType");
