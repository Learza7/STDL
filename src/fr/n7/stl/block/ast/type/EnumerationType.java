...
public class EnumerationType implements Type, Declaration {
	...
	public boolean equalsTo(Type _other) {
		...
		if (_other instanceof EnumerationType) {
			EnumerationType _otherEnum = (EnumerationType) _other;
			if (this.labels.size() == _otherEnum.labels.size()) {
				Iterator<LabelDeclaration> _iter = this.labels.iterator();
				Iterator<LabelDeclaration> _otherIter = _otherEnum.labels.iterator();
				while (_iter.hasNext()) {
					result = result && _iter.next().equals(_otherIter.next());
				}
			} else {
				result = false;
			}
